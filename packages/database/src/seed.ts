...
const SEED_WORKFLOWS = [
  {
    ...
    description: 'Scrapes databases for latest research papers, scans patents, and synthesizes findings using a Consensus Supervisor.',
    ...
    workflowData: {
      nodes: [
        { id: 'node-trigger', type: 'trigger', position: { x: 50, y: 200 }, data: { label: 'Web Trigger (Cron 9 AM)', outputFormat: 'text' } },
  ...
  },
  {
    ...
    description: 'Automates multi-language translation and context review before publishing localized files to the directory.',
  ...
  },
  {
    ...
    description: 'Performs static analysis of files, checks for exposed secrets/API keys, and submits risk tickets automatically.',
    categorySlug: 'devops',
    ...
    workflowData: {
      nodes: [
        { id: 'node-trigger-audit', type: 'input_trigger', position: { x: 50, y: 200 }, data: { label: 'Audit directory: packages/database/src/schema', outputFormat: 'text' } },
      ...
      edges: [
        ...
        { id: 'e-ta-ss', source: 'node-trigger-audit', target: 'node-secret-scanner', type: 'animated' },
        { id: 'e-sa-su', source: 'node-static-analyzer', target: 'node-sec-supervisor', type: 'animated' },
        { id: 'e-ss-su', source: 'node-secret-scanner', target: 'node-sec-supervisor', type: 'animated' },
        { id: 'e-su-jr', source: 'node-sec-supervisor', target: 'node-jira-reporter', type: 'animated' },
      ],
    },
  },
  {
    name: 'Customer Support Escalation Swarm',
    description: 'Classifies customer issues, runs specialized diagnosis (billing & DevOps) concurrently, constructs a polished resolution message, and logs results to CRM & Slack.',
    ...
    workflowData: {
      nodes: [
        { id: 'node-trigger-support', type: 'input_trigger', position: { x: 50, y: 250 }, data: { label: 'Customer complaint: Received double charge on last invoice and API latency is above 2000ms.', outputFormat: 'text' } },
        { id: 'node-support-triage', type: 'supervisor', position: { x: 300, y: 250 }, data: { label: 'Triage Supervisor', model: 'qwen3-max', systemPrompt: 'Analyze client issues. Trigger Billing Specialist for charges or invoices. Trigger DevOps Specialist for timeouts or high latency. Trigger general Support Agent for other queries.', enableThinking: true, thinkingBudget: 1024, outputFormat: 'json' } },
        { id: 'node-billing-agent', type: 'agent', position: { x: 550, y: 100 }, data: { label: 'Billing Specialist', model: 'qwen-plus', systemPrompt: 'Search invoice history database, identify duplicate billing charges, and calculate refunds.', outputFormat: 'markdown' } },
      ...
      edges: [
        ...
        { id: 'e-st-ba', source: 'node-support-triage', target: 'node-billing-agent', type: 'animated' },
        { id: 'e-st-da', source: 'node-support-triage', target: 'node-devops-agent', type: 'animated' },
        { id: 'e-st-fa', source: 'node-support-triage', target: 'node-feedback-agent', type: 'animated' },
        { id: 'e-ba-rr', source: 'node-billing-agent', target: 'node-resolution-reviewer', type: 'animated' },
        { id: 'e-da-rr', source: 'node-devops-agent', target: 'node-resolution-reviewer', type: 'animated' },
        { id: 'e-fa-rr', source: 'node-feedback-agent', target: 'node-resolution-reviewer', type: 'animated' },
        { id: 'e-rr-db', source: 'node-resolution-reviewer', target: 'node-db-writer', type: 'animated' },
        { id: 'e-rr-sp', source: 'node-resolution-reviewer', target: 'node-slack-publisher', type: 'animated' },
      ],
    },
  },
  {
    name: 'Social Media Content Scheduler',
    description: 'Generates on-brand social posts from a brief, reviews tone, and schedules them across platforms via MCP tools.',
    categorySlug: 'content',
    ...
    workflowData: {
      ...
      edges: [
        ...
        { id: 'e-cw-br', source: 'node-copywriter', target: 'node-brand-reviewer', type: 'animated' },
        { id: 'e-br-ms', source: 'node-brand-reviewer', target: 'node-mcp-scheduler', type: 'animated' },
      ],
    },
  },
  {
    name: 'Code Review Pipeline',
    description: 'Reviews pull requests for code quality, security issues, and best practices before merging.',
    categorySlug: 'devops',
    ...
    workflowData: {
      nodes: [
        { id: 'node-trigger-cr', type: 'trigger', position: { x: 50, y: 200 }, data: { label: 'PR Merged Webhook', outputFormat: 'text' } },
      ...
      edges: [
        ...
        { id: 'e-tcr-vs', source: 'node-trigger-cr', target: 'node-vulnerability', type: 'animated' },
        { id: 'e-la-cs', source: 'node-linter', target: 'node-cr-supervisor', type: 'animated' },
        { id: 'e-vs-cs', source: 'node-vulnerability', target: 'node-cr-supervisor', type: 'animated' },
        { id: 'e-cs-gw', source: 'node-cr-supervisor', target: 'node-github-writer', type: 'animated' },
      ],
    },
  },
  {
    name: 'Data Pipeline Orchestrator',
    description: 'Extracts data from APIs, transforms it, and loads it into a database with quality checks at each stage.',
    categorySlug: 'data',
    ...
    workflowData: {
      nodes: [
        { id: 'node-trigger-dp', type: 'trigger', position: { x: 50, y: 200 }, data: { label: 'Daily Cron (2 AM)', outputFormat: 'text' } },
      ...
      edges: [
        ...
        { id: 'e-tr-qg', source: 'node-transformer', target: 'node-quality-gate', type: 'animated' },
  ...
  },
  {
    ...
    description: 'Scores inbound leads, enriches with public data, and pushes qualified leads to the CRM.',
    categorySlug: 'customer-support',
    ...
    workflowData: {
      nodes: [
        { id: 'node-trigger-ls', type: 'trigger', position: { x: 50, y: 200 }, data: { label: 'Webhook: New Lead Form Submission', outputFormat: 'text' } },
        { id: 'node-scorer', type: 'agent', position: { x: 300, y: 200 }, data: { label: 'Lead Scorer', model: 'qwen-plus', systemPrompt: 'Score the lead based on company size, industry, and engagement signals. Return score 0-100.', outputFormat: 'json' } },
        { id: 'node-enricher', type: 'agent', position: { x: 550, y: 200 }, data: { label: 'Data Enricher', model: 'qwen-plus', systemPrompt: 'Look up company information: headcount, funding, tech stack from public sources.', outputFormat: 'json' } },
        { id: 'node-routing-agent', type: 'supervisor', position: { x: 800, y: 200 }, data: { label: 'Routing Supervisor', model: 'qwen3-max', systemPrompt: 'If lead score > 70 route to sales. If 40-70 route to nurture. If < 40 archive.', enableThinking: true, thinkingBudget: 512, outputFormat: 'json' } },
      ],
      edges: [
...
const CATEGORIES = [
  ...
  { id: id('cat'), name: 'Customer Support', slug: 'customer-support', icon: 'Users', sortOrder: 4 },
...
const REVIEWS_DATA: { templateIdx: number; rating: number; review: string }[] = [
  { templateIdx: 0, rating: 5, review: 'Excellent research workflow. The consensus supervisor works great for literature reviews.' },
  { templateIdx: 0, rating: 4, review: 'Very useful for automating paper summaries. Would love more output format options.' },
  { templateIdx: 1, rating: 5, review: 'Perfect for our i18n pipeline. Saved us hours of manual translation work.' },
  { templateIdx: 1, rating: 5, review: 'The linguistic reviewer catches nuances that other tools miss. Highly recommend.' },
  { templateIdx: 2, rating: 4, review: 'Good security audit tool. Could use more vulnerability database integrations.' },
  { templateIdx: 3, rating: 5, review: 'Our support team loves this. Reduced ticket resolution time by 60%.' },
  { templateIdx: 3, rating: 4, review: 'Great escalation workflow. The triage supervisor is very accurate.' },
  { templateIdx: 3, rating: 3, review: 'Works well but needs more CRM integrations out of the box.' },
  { templateIdx: 4, rating: 3, review: 'Decent scheduler but the brand voice detection could be better.' },
  { templateIdx: 5, rating: 4, review: 'Excellent code review automation. Catches things our team misses.' },
  { templateIdx: 5, rating: 5, review: 'The vulnerability scanner found issues in our dependencies. Very thorough.' },
  { templateIdx: 6, rating: 3, review: 'Good ETL pipeline but needs more data source connectors.' },
  { templateIdx: 7, rating: 4, review: 'Our sales team qualified 3x more leads after implementing this workflow.' },
];
...
async function seed() {
  ...
  sqliteDb.insert(s.sqliteUsers).values({
    id: userId,
  ...
  for (const cat of CATEGORIES) {
    sqliteDb.insert(s.sqliteTemplateCategories).values({
      id: cat.id,
      name: cat.name,
      slug: cat.slug,
      icon: cat.icon,
  ...
  for (const wf of SEED_WORKFLOWS) {
    ...
    sqliteDb.insert(s.sqliteTemplates).values({
      id: tid,
      name: wf.name,
  ...
  for (const r of REVIEWS_DATA) {
    sqliteDb.insert(s.sqliteTemplateReviews).values({
      id: id('rev'),
