  ...
  sendCopilotMessage: async (text) => {
    ...
    setTimeout(() => {
      ...
      if (
        ...
        get().clearGraph();
        reply =
          'Canvas cleared. You can start fresh or drag and drop items from the Left Component Palette.';
      } else if (promptLower.includes('model') || promptLower.includes('qwen')) {
        reply =
          'We support Alibaba Cloud Qwen models: `qwen-turbo` (fast, efficient Workers) and `qwen3-max` (highly logical Supervisor nodes with thinking budgets enabled).';
