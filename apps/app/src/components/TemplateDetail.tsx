    ...
    <div className="h-screen w-screen flex flex-col bg-[#f8fafc] text-slate-800 select-none overflow-hidden font-sans">
      <header className="h-14 bg-white border-b border-[#cbd5e1] flex items-center justify-between px-6 z-20 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {forking ? <Loader2 className="w-3 h-3 animate-spin" /> : <ExternalLink className="w-3.5 h-3.5" />}
            FORK TO WORKSPACE
          </button>
        </div>
      </header>
        ...
        <div className="max-w-3xl mx-auto space-y-8">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {template.tags.map((tag) => (
                    <span key={tag} className="px-2 py-0.5 bg-slate-50 border border-slate-200 text-[10px] font-mono text-slate-500">
                      #{tag}
                    </span>
                  ))}
                </div>
          ...
          <div className="border-t border-slate-200 pt-6">
            ...
            <div className="space-y-2">
              {(template.workflowData?.nodes || []).map((node: any) => (
                <div key={node.id} className="flex items-center gap-3 bg-white border border-slate-200 px-4 py-2.5">
                  <span className={`text-[9px] font-mono font-bold uppercase px-1.5 py-0.5 ${
                    ...
                    {node.type === 'trigger' ? 'trigger' :
                     node.type === 'input_trigger' ? 'input' :
                     node.type === 'mcp_tool' ? 'mcp' : node.type}
                  </span>
                  <span className="text-xs font-semibold text-slate-700">
                    {node.data?.label || node.id}
                  </span>
                  {node.data?.model && (
                    <span className="text-[9px] font-mono text-slate-400 ml-auto">{node.data.model}</span>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="border-t border-slate-200 pt-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                {forking ? <Loader2 className="w-4 h-4 animate-spin" /> : <ExternalLink className="w-4 h-4" />}
                Fork to Workspace
              </button>
